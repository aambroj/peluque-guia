...
export default async function ExportarReservasPage() {
  ...
  return (
    <section className="px-6 py-8">
      <div className="mx-auto max-w-3xl space-y-8">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-zinc-600">
                Elige un rango de fechas y, si quieres, filtra por estado,
                empleado y servicio antes de descargar el archivo CSV.
              </p>
            </div>
        ...
        {serviciosError ? (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            Error cargando servicios: {serviciosError.message}
          </div>
        ) : null}

        <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
          <form
            action="/api/export/reservas"
            method="GET"
            className="space-y-6"
          >
                ...
                >
                  ...
                  {(empleados ?? []).map((empleado: any) => (
                    <option key={empleado.id} value={empleado.id}>
                      {empleado.name ?? `Empleado ${empleado.id}`}
                    </option>
                  ))}
                </select>
              </div>
                ...
                >
                  ...
                  {(servicios ?? []).map((servicio: any) => (
                    <option key={servicio.id} value={servicio.id}>
                      {servicio.name ?? `Servicio ${servicio.id}`}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600">
              Consejo: para facturación o análisis, normalmente interesa
              exportar por mes, por estado, por profesional o por servicio.
            </div>
